<?php
// Script para verificar as permissões dos arquivos no servidor (versão simplificada)

// Informações do sistema
echo "<h1>Verificação de Permissões (Simplificada)</h1>";
...
echo "<tr><th>Arquivo</th><th>Tipo</th><th>Permissões</th><th>Tamanho</th><th>Modificado</th></tr>";

$files = scandir('.');
foreach ($files as $file) {
    if ($file != '.' && $file != '..') {
        $perms = substr(sprintf('%o', fileperms($file)), -4);
        // Removidas as chamadas para posix_getpwuid e posix_getgrgid
        $size = filesize($file);
...
// Verificar o caminho do GitHub Actions
echo "<h2>Verificação do Caminho do GitHub Actions</h2>";
$github_dir = '/www/wwwroot/lotominas.site/';
if (is_dir($github_dir)) {
    echo "<p style='color: green;'>✅ O diretório <strong>$github_dir</strong> existe.</p>";
    echo "<p><strong>Permissões:</strong> " . substr(sprintf('%o', fileperms($github_dir)), -4) . "</p>";
} else {
    echo "<p style='color: red;'>❌ O diretório <strong>$github_dir</strong> não existe.</p>";
}
